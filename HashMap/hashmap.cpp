...
HashMap<K, M, H>::HashMap(const HashMap& rhs):
    // dont need one by one, use ctor
    HashMap{rhs.bucket_count(), rhs._hash_function} {
    
    for(auto& KM : rhs) insert(KM);
    /* rhs itself is a series of {K, M}, dont dig in bucket node.
    
    for(auto &curr : rhs._buckets_array) {
        auto tmp = curr;
        while(tmp) {
            insert(tmp->value);
            tmp = tmp->next;
...
HashMap<K, M, H>::HashMap(HashMap&& rhs) :
    ...
    rhs._size = 0; // after move, the onwership of rhs gone, should be inserted from size 0.
}
...
void HashMap<K, M, H>::rehash(size_t new_bucket_count) {
    if (new_bucket_count == 0) {
        throw std::out_of_range(
            "HashMap<K, M, H>::rehash: new_bucket_count must be positive.");
    }

    std::vector<node*> new_buckets_array(new_bucket_count, nullptr);
    for (auto& curr :
         _buckets_array) {  // short answer question is asking about this 'curr'
        while (curr != nullptr) {
            const auto& [key, mapped] = curr->value;
